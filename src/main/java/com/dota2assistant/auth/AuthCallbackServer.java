    ...
    private static class CallbackHandler implements HttpHandler {
        ...
        private String createSuccessPage() {
            return "<html><head><title>Authentication Successful</title>"
                    ...
                    + ".close-btn { background-color: #4CAF50; color: white; border: none; padding: 15px 30px; "
                    + "text-align: center; text-decoration: none; display: inline-block; font-size: 18px; font-weight: bold; "
                    + "margin: 20px 2px; cursor: pointer; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); "
                    + "transition: all 0.3s; }"
                    + ".close-btn:hover { background-color: #45a049; transform: translateY(-2px); box-shadow: 0 6px 8px rgba(0,0,0,0.15); }"
                    + ".close-btn:active { transform: translateY(0px); }"
                    + ".message { font-size: 16px; margin: 15px 0; }"
                    + ".note { font-size: 14px; color: #666; margin-top: 20px; }"
                    + "@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.6; } }"
                    + ".close-btn { animation: pulse 2s infinite; }"
                    + "</style></head>"
                    + "<body><div id='main-container' class='container'>"
                    + "<h1>✓ Authentication Successful!</h1>"
                    + "<p class='message'>You have successfully logged in with Steam.</p>"
                    + "<p class='message'>The application has received your login information.</p>"
                    + "<button class='close-btn' onclick='attemptClose();'>Close This Tab</button>"
                    + "<p class='note'>This page will redirect to a blank page in 3 seconds.<br>You can safely close this tab now.</p>"
                    + "<script>"
                    ...
                    + "}"
                    + "// Auto-attempt close after a moment"
                    + "setTimeout(function() { attemptClose(); }, 2000);"
                    + "</script>"
                    + "</div></body></html>";
        }
